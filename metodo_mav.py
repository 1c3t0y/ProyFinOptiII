...
def metodo_MAV(prob_transporte):
    ...
    penalizacion_ren = np.tile(0.0, prob_transporte.n)
    penalizacion_col = np.tile(0.0, prob_transporte.m)
    penalizacion_mayor = (
        0.0,
        False,
        -1,
    )  ###(Valor penalizacion, False = renglon / True = columna, indice)
    ...
    while not (
        ...
        for i, renglon in enumerate(prob_transporte.matriz_costos):
            ...
            for j, elemento in enumerate(renglon):
                if not taches_ren[i] and not taches_col[j] and elemento <= costo_menor:
                    ...
                    not taches_ren[i]
                    and not taches_col[j]
                    and elemento <= seg_costo_menor
                ):
        ...
        if not penalizacion_mayor[1]:
            for j, elemento in enumerate(
                prob_transporte.matriz_costos[penalizacion_mayor[2]]
            ):
                if not taches_col[j] and elemento <= costo_menor[0]:
                    costo_menor = (elemento, penalizacion_mayor[2], j)
        else:
            for i, elemento in enumerate(
                prob_transporte.matriz_costos.transpose()[penalizacion_mayor[2]]
            ):
        ...
        prob_transporte.matriz_variables_decision[costo_menor[1]][
            costo_menor[2]
        ] = menor
        ...
        if num_taches_ren == 1 and num_taches_col == 1 and np.sum(oferta_aux) > 0:
            ...
            prob_transporte.matriz_variables_decision[costo_menor[1]][
                costo_menor[2]
            ] = oferta_aux[costo_menor[1]]
            prob_transporte.matriz_variables_basicas[costo_menor[1]][
                costo_menor[2]
            ] = True
            demanda_aux[costo_menor[2]] = (
                demanda_aux[costo_menor[2]] - oferta_aux[costo_menor[1]]
            )
            ...
            costo_menor = (
                np.amax(prob_transporte.matriz_costos.transpose()[col_faltante]),
                -1,
                -1,
            )
            for i, elemento in enumerate(
                prob_transporte.matriz_costos.transpose()[col_faltante]
            ):
                if not taches_ren[i] and elemento <= costo_menor[0]:
            ...
            oferta_aux[costo_menor[1]] = (
                oferta_aux[costo_menor[1]] - demanda_aux[costo_menor[2]]
            )
        ...
        while (
            np.sum(oferta_aux) == 0
            and np.sum(demanda_aux) == 0
            and num_taches_ren > 0
            and num_taches_col > 0
        ):
                ...
                for j, columna in enumerate(prob_transporte.matriz_costos.transpose()):
                    ...
                    for j, elemento in enumerate(renglon):
                        if (
                            not taches_ren[i]
                            and not taches_col[j]
                            and elemento <= costo_menor
                        ):
                            costo_menor = (elemento, i, j)
                    if (taches_col == False).sum() > 1:
                        prob_transporte.matriz_costos[costo_menor[1]][
                            costo_menor[2]
                        ] = 0
                        oferta_aux[i] = True
                num_taches_col = (taches_col == False).sum()
